...
<template>
  <div v-if="currentUser">
    ...
    <div class="flex flex-col gap-3">
      ...
      <Input label="Partner/PSP ID" v-model="partnerID" @keyup.enter="generateToken" />

      <Input label="Merchant Username" v-model="merchantUsername" @keyup.enter="generateToken" />

      <Input type="password" label="Secure Key" v-model="merchantSecureKey" @keyup.enter="generateToken" />

      <button class="btn btn-primary" :class="{ loading: isLoading }" @click="generateToken">
        Generate Token
      </button>
      ...
      <button v-if="responseData.AuthToken" class="btn w-full gap-2" @click="copyString(responseData.AuthToken)">
